...
resource "aws_s3_bucket" "mybucket" { # here we are using the s3 bucket and naming it mybucket
    ...
    bucket = "my-unique-bucket-name-${terraform.workspace}-${random_id.rand_id.hex}" # this is for the bucket name which should be unique across all aws users  and the random id will generate the random password in this bucket 
     #so this terraform.workspace will give me the name of the workspace which i am using like dev, prod ,test
    tags = {
